# ES.OCR.ARYA.SYS.PREDICT #
Based on [InvoiceNet](https://github.com/naiveHobo/InvoiceNet) - Based on [1] Palm, Rasmus Berg, Florian Laws, and Ole Winther. "Attend, Copy, Parse End-to-end information extraction from documents." 2019 International Conference on Document Analysis and Recognition (ICDAR). IEEE, 2019.

We have added a Web Server for Prediction over it. Which can be started after installing the solution - 

`$ python prediction_http_server.py`

and `/predict` endpoint will be exposed.  Our custom codes can be found in `webapp` folder.
Our training json data fieldnames and format is:
...
# config in invoicenet/__init__.py
FIELDS["filename"] = FIELD_TYPES["general"]
FIELDS["transmittername"] = FIELD_TYPES["general"]
FIELDS["transmittervif"] = FIELD_TYPES["general"]
FIELDS["receptorname"] = FIELD_TYPES["general"]
FIELDS["receptorcif"] = FIELD_TYPES["general"]
FIELDS["invoicedate"] = FIELD_TYPES["date"]
FIELDS["invoicenumber"] = FIELD_TYPES["general"]
FIELDS["concept"] = FIELD_TYPES["general"]
FIELDS["imponiblebase"] = FIELD_TYPES["general"]
FIELDS["total"] = FIELD_TYPES["amount"]
FIELDS["paymenttype"] = FIELD_TYPES["general"]
```


For installation please read below:
